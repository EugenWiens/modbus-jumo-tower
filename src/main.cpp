...
void loop() {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        if (g_modbus.holdingRegs[i]     != s_prevRegs[i])     disp1Changed = true;
        if (g_modbus.holdingRegs[i + 8] != s_prevRegs[i + 8]) disp2Changed = true;
    }

    if (disp1Changed || disp2Changed) {
        memcpy(s_prevRegs, g_modbus.holdingRegs, sizeof(s_prevRegs));
        if (disp1Changed) refreshDisplay(0);
        if (disp2Changed) refreshDisplay(1);
    }
}
